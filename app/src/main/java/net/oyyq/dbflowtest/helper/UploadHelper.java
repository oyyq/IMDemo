package net.oyyq.dbflowtest.helper;
...
import net.oyyq.common.util.HashUtil;
...
public class UploadHelper {
    ...
    // 上传的仓库名
    private static final String BUCKET_NAME =  "italker-oyyq-test";
    private static OSSClient oss = getClient();



    private static OSSClient getClient() {
        // 明文设置secret的方式建议只在测试时使用，更多鉴权模式请参考后面的`访问控制`章节,
        // TODO 现在我使用的是root accesskeyid, accesskeysecret, (RAM子账户权限有问题)
        OSSCredentialProvider credentialProvider = new OSSPlainTextAKSKCredentialProvider(
                "","");

        return new OSSClient(DemoApplication.getInstance(), ENDPOINT, credentialProvider);
    }
     ...
     * @param path    需要上传的文件的路径, 那么你需要上传的还是一个File类型
     * @return 存储的地址
     */
    private static String upload(String objKey, String path) {
        ...
        OSSCompletedCallback<PutObjectRequest, PutObjectResult> putCallback = new OSSCompletedCallback<PutObjectRequest, PutObjectResult>(){
        ...
        OSSAsyncTask task = oss.asyncPutObject(request, putCallback);
