package net.oyyq.dbflowtest.fragment.assist;
...
public class PermissionsFragment extends BottomSheetDialogFragment implements EasyPermissions.PermissionCallbacks {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        return new GalleryFragment.TransStatusBottomSheetDialog(getContext());          // 复用即可
    }
    ...
    public void onResume() {
        super.onResume();
        // 界面显示的时候进行刷新, getView(): 返回Fragment的根布局
     ...
     * @return 是否有权限
     */
    public static boolean haveAll(Context context, FragmentManager manager, outerCallback outer) {
        PermissionsFragment.outer = outer;
        // 检查是否具有所有的权限
    ...
    private void requestPerm() {
        ...
        if (EasyPermissions.hasPermissions(getContext(), perms)) {
            DemoApplication.showToast(R.string.label_permission_ok);
    ...
    //LaunchActivity implements outerCallback
    public interface outerCallback{
        void notifyPermission(boolean succeed);
    }

    private static outerCallback outer;

    public void setOuter(outerCallback outer) {
        this.outer = outer;
    }
}
